...
export async function GET(request: NextRequest) {
  try {
    ...
    // Оптимизированный запрос БЕЗ фотографий для быстрой загрузки
    ...
    // Парсим даты и добавляем пустой массив фотографий для оптимизации
    const parsedData = data.map(item => ({
      ...item, 
      orderDate: new Date(item.orderDate),
      photos: [] // Пустой массив для быстрой загрузки
    }));

    console.log(`✅ Admin API: Заказы получены: ${parsedData.length} шт. (без фотографий)`);
